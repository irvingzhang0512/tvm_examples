# https://github.com/Hagaik92/FastDepth/blob/main/models.py
import math
import os
...
def DWConvReLU6Block(in_channels,
                     out_channels,
                     kernel_size,
                     stride,
                     padding=None):
...
class MobileNet_Encoder(nn.Module):
    def __init__(self):
        ...
        self.enc_layer2 = nn.Sequential(DWConvReLU6Block(32, 32, 3, 1, 1),
                                        ConvReLU6Block(32, 64, 1, 1, 0))
        self.enc_layer3 = nn.Sequential(DWConvReLU6Block(64, 64, 3, 2, 1),
                                        ConvReLU6Block(64, 128, 1, 1, 0))
        self.enc_layer4 = nn.Sequential(DWConvReLU6Block(128, 128, 3, 1, 1),
                                        ConvReLU6Block(128, 128, 1, 1, 0))
        self.enc_layer5 = nn.Sequential(DWConvReLU6Block(128, 128, 3, 2, 1),
                                        ConvReLU6Block(128, 256, 1, 1, 0))
        self.enc_layer6 = nn.Sequential(DWConvReLU6Block(256, 256, 3, 1, 1),
                                        ConvReLU6Block(256, 256, 1, 1, 0))
        self.enc_layer7 = nn.Sequential(DWConvReLU6Block(256, 256, 3, 2, 1),
                                        ConvReLU6Block(256, 512, 1, 1, 0))
        self.enc_layer8 = nn.Sequential(DWConvReLU6Block(512, 512, 3, 1, 1),
                                        ConvReLU6Block(512, 512, 1, 1, 0))
        self.enc_layer9 = nn.Sequential(DWConvReLU6Block(512, 512, 3, 1, 1),
                                        ConvReLU6Block(512, 512, 1, 1, 0))
        self.enc_layer10 = nn.Sequential(DWConvReLU6Block(512, 512, 3, 1, 1),
                                         ConvReLU6Block(512, 512, 1, 1, 0))
        self.enc_layer11 = nn.Sequential(DWConvReLU6Block(512, 512, 3, 1, 1),
                                         ConvReLU6Block(512, 512, 1, 1, 0))
        self.enc_layer12 = nn.Sequential(DWConvReLU6Block(512, 512, 3, 1, 1),
                                         ConvReLU6Block(512, 512, 1, 1, 0))
        self.enc_layer13 = nn.Sequential(DWConvReLU6Block(512, 512, 3, 2, 1),
                                         ConvReLU6Block(512, 1024, 1, 1, 0))
        self.enc_layer14 = nn.Sequential(DWConvReLU6Block(1024, 1024, 3, 1, 1),
                                         ConvReLU6Block(1024, 1024, 1, 1, 0))
...
class NNConv5_Decoder(nn.Module):
    def __init__(self, kernel_size, depthwise=True):
        ...
        if (depthwise):
            self.conv1 = nn.Sequential(DWConvBlock(1024, 1024, kernel_size, 1),
                                       ConvBlock(1024, 512, 1, 1, 0))
            self.conv2 = nn.Sequential(DWConvBlock(512, 512, kernel_size, 1),
                                       ConvBlock(512, 256, 1, 1, 0))
            self.conv3 = nn.Sequential(DWConvBlock(256, 256, kernel_size, 1),
                                       ConvBlock(256, 128, 1, 1, 0))
            self.conv4 = nn.Sequential(DWConvBlock(128, 128, kernel_size, 1),
                                       ConvBlock(128, 64, 1, 1, 0))
            self.conv5 = nn.Sequential(DWConvBlock(64, 64, kernel_size, 1),
            ...
            self.conv2 = ConvBlock(512, 256, kernel_size, 1,
                                   (kernel_size - 1) // 2)
            self.conv3 = ConvBlock(256, 128, kernel_size, 1,
                                   (kernel_size - 1) // 2)
            self.conv4 = ConvBlock(128, 64, kernel_size, 1,
                                   (kernel_size - 1) // 2)
            self.conv5 = ConvBlock(64, 32, kernel_size, 1,
                                   (kernel_size - 1) // 2)
...
class NNConv5_DecoderV2(nn.Module):
    def __init__(self, kernel_size, depthwise=True):
        ...
        if (depthwise):
            self.conv1 = nn.Sequential(DWConvBlock(320, 320, kernel_size, 1),
                                       ConvBlock(320, 96, 1, 1, 0))  # 14X14
            self.conv2 = nn.Sequential(DWConvBlock(96, 96, kernel_size, 1),
                                       ConvBlock(96, 32, 1, 1, 0))  # 28 X 28
            self.conv3 = nn.Sequential(DWConvBlock(32, 32, kernel_size, 1),
                                       ConvBlock(32, 24, 1, 1, 0))  # 56X56
            self.conv4 = nn.Sequential(DWConvBlock(24, 24, kernel_size, 1),
                                       ConvBlock(24, 16, 1, 1, 0))  # 112 X 112
            self.conv5 = nn.Sequential(DWConvBlock(16, 16, kernel_size, 1),
                                       ConvBlock(16, 32, 1, 1, 0))  # 224 X 224
...
class FastDepth(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.interpolate(self.decoder.conv1(x),
                          scale_factor=2,
                          mode='nearest')
        x = F.interpolate(self.decoder.conv2(x),
                          scale_factor=2,
                          mode='nearest')
        x = x + layer3
        x = F.interpolate(
            self.decoder.conv3(x), scale_factor=2, mode='nearest') + layer2
        x = F.interpolate(
            self.decoder.conv4(x), scale_factor=2, mode='nearest') + layer1
        x = F.interpolate(self.decoder.conv5(x),
                          scale_factor=2,
                          mode='nearest')
...
class FastDepthV2(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.interpolate(self.decoder.conv2(x),
                          ...
                          mode='nearest')
        x = x + layer3
        x = F.interpolate(self.decoder.conv3(x),
                          scale_factor=2,
                          mode='nearest')
        x = x + layer2
        x = F.interpolate(self.decoder.conv4(x),
                          scale_factor=2,
                          mode='nearest')
        x = x + layer1
        x = F.interpolate(self.decoder.conv5(x),
...
def get_scripted_moidel(model_type, pth_path, input_shape=[1, 3, 224, 224]):
    if model_type == 'v1':
        model_fn = FastDepth
    elif model_type == 'v2':
        model_fn = FastDepthV2
    else:
        raise ValueError(f"unknown model type {model_type}")
    model = model_fn().eval()
    load_pretrained_fastdepth(model, pth_path)
    input_data = torch.randn(input_shape)
    return torch.jit.trace(model, input_data).eval()
...
if __name__ == '__main__':
    ...
    get_scripted_moidel('v1', '../data/fastdepth/FastDepth_L1GN_Best.pth')
    print("v1 successfully")

    get_scripted_moidel('v2', '../data/fastdepth/FastDepthV2_L1GN_Best.pth')
    print("v2 successfully")
